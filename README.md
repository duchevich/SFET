...
- [Конфигурированте SFET](#Конфигурированте-SFET)
- [Декларирование блоков](#Декларирование-блоков)
- [Планы к релизу мажорной версии](#Планы-к-релизу-мажорной-версии)
- [Авторы](#Авторы)
...
- `sources/` - директория исходных файлов проекта.
    - `{blockName}/` - директория блока. Блоки имеющие в имени подчеркивание "_" в качестве префикса являються core-блоками.
    ...
    - `_bases/` - core-блок содержащий базовые стили, скрипты, нормалайзеры и модернайзеры.
    - `_grid/` - core-блок с реализацией сетки.
...
## Декларирование блоков
Файл `declaration.js` существует для быстрого и удобного декларирования проектных блоков. Ключ `n` (name) - имя блока, ключ `v` (vendors) - массив имен поставщиков блока. Декларировать блоки следует в следующем формате:
``js
{n:'{blockName}', v:['{vendor1Name}', '{vendor2Name}', '{vendor3Name}']},
``
*Внимание!* Последовательность блоков и поставщиков в декларации указывает на порядок склейки кода.


## Планы к релизу мажорной версии
- [x] Наполнить коллекцию миксинов.
- [ ] Найти замену блоку `grid` построенную на flexbox.
- [ ] Реализовать возможность разбивать код блоков по типу происхождения на файлы в соответствующем порядке склейки: код ядра блока `{blockName}.core.*`, по-умолчанию `{blockName}.*` (в том чиле для обратной совместимости с блоками старых версий SFET), код темы блока `{blockName}.theme.*`, произвольных код блока `{blockName}.custom.*`.
- [ ] Создать в `gulpfile.js` функцию для работы с sass, дать возможность выбора в `config.js` между less и sass.
- [ ] Реализовать возможность разбивать блоки на бандлы.
